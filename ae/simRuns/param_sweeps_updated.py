...
def sweep_params(tag, memory_capacity, memBW, interconnectBW, lock):
    # FIXME: Really? Running in different dir and saving results in some other?
    # FIXME: REALLY?
    # INFO: Done !!!
    resultsDir = "ae/simRuns/results_updated"
    ...
    with lock:
        ...
        with open(
            f"{resultsDir}/{tag}/memory_{_memName}_{_stacks}_bw_{_interConnectName}_results_bs{batch_size}_init.csv",
            "a",
        ) as f:
            f.write(
        ...
        with open(
            f"{resultsDir}/{tag}/memory_{_memName}_{_stacks}_bw_{_interConnectName}_results_bs{batch_size}_ar.csv",
            "a",
        ) as f:
            f.write(
...
if __name__ == "__main__":
    ...
    #     _memName = defaults[1]["Name"]
    #     _stacks = defaults[1]["Stacks"]
    #     _interConnectName = defaults[2]["Name"]
    #
    #     interconnectSen.append(
    #         f"memory_{_memName}_{_stacks}_bw_{_interConnectName}_results_bs{batch_size}"
    #     )
    #     processes.append(
    #         Process(
    #             target=sweep_params,
    #             args=("sensitivity", defaults[0], defaults[1], cCap, lock),
    #         )
    #     )
    #
    # # Memory sensitivity
    ...
    #         Process(
    #             target=sweep_params,
    #             args=("sensitivity", defaults[0], cMem, defaults[2], lock),
    #         )
    #     )
    ...
    for cInterconnect in interconnectType:
        for cMem in memoryType:
            ...
            processes.append(
                Process(
                    target=sweep_params,
                    args=("sweep", defaults[0], cMem, cInterconnect, lock),
                )
            )
    ...
    resultsDir = "ae/simRuns/results_updated"
    # with open(f"{resultsDir}/sensitivity/simRuns.txt", "w") as _file:
    #     _file.writelines("\n".join(memSen))
    # with open(f"{resultsDir}/sensitivity/simRuns.txt", "w") as _file:
    #     _file.writelines("\n".join(interconnectSen))
    with open(f"{resultsDir}/sweep/simRuns.txt", "w") as _file:
        _file.writelines("\n".join(sweepConfis))
